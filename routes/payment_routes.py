...
import requests
from config import Config  # Import the Config class, not the individual values
...
def create_payment_request(order_id, amount, currency="USD", payment_method="mobile_money"):
    headers = {
        "Authorization": f"Bearer {Config.PESAPAL_API_KEY}",  # Access through Config
    ...
    response = requests.post(Config.PESAPAL_API_URL, json=data, headers=headers)  # Access through Config
